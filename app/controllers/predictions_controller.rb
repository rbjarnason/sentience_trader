class PredictionsController < ApplicationController
  ...
  def index
    unless params[:only_id]
      @predictions = Prediction.find(:all, :order=>"neural_strategy_id ASC, data_time ASC",:limit=>250)
      @neural_strategy = NeuralStrategy.find(:first)
    else
      @predictions = Prediction.find(:all, :conditions=>"neural_strategy_id = #{params[:only_id]}")
      @neural_strategy = NeuralStrategy.find(params[:only_id])
      @neural_strategy.demarshall
    end
